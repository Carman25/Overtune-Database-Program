...
CREATE TABLE Track (
    ...
    mode              TEXT,
    -- ISRC (International Standard Recording Code): fixed 12-character identifier
    -- per ISO 3901 (e.g. 'USUM71703861').
    -- UNIQUE: one canonical ISRC per track row; if a recording has multiple ISRCs
    -- the ETL picks the earliest one. Additional ISRCs can be stored in a separate
    -- Track_ISRC table if full multi-ISRC support is needed.
    isrc              CHAR(12) UNIQUE,
    explicit_flag     BOOLEAN,
...
-- Track.isrc already has a UNIQUE constraint (which creates an index),
-- so no separate index is needed here.
